...
public class searchListDataTest {
	...
	public void searchTest() throws ParseException {
		...
		long startTime = DateUtils.parseDate("2018-01-29 00:00:00", "yyyy-MM-dd HH:mm:ss").getTime();
		long endTime = DateUtils.parseDate("2018-01-30 00:00:00", "yyyy-MM-dd HH:mm:ss").getTime();
	 ...
	 */
	public String searchListData(String index, String type, long startTime, long endTime,
			Integer size, String matchStr) throws ParseException {

		StringBuffer stringBuffer = new StringBuffer();

		List<Map<String, Object>> mapList = ElasticSearchUtils
				.searchListData(index, type, startTime, endTime, size, matchStr);

		Set<String> guidList = new HashSet<String>();

		Set<String> requestIdList = new HashSet<String>();

		for (int i = 0, count = mapList.size(); i < count; i++) {
			Map<String, Object> stringObjectMap = mapList.get(i);
			String message = stringObjectMap.get("message").toString();
		...
		for (String requestId : requestIdList) {
			...
			List<Map<String, Object>> tmpMap3 = ElasticSearchUtils
					.searchListData(assignservice_index, assignservice_type, startTime, endTime, size,
							assignservice_matchStr);
				...
				if (message.indexOf("返回结果为：{") > 1) {
					...
					if (recommendDabi != null) {
						...
						if (startDate != null) {
							...
							stringBuffer.append(recommendDabi == null ? "" : recommendDabi).append(" ")
